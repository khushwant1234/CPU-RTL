//=============================================================================
// tb_reg_file.sv  -  unit test for reg_file
...
module tb_reg_file;
  ...
  logic        rst_n;
  logic        clk = 0, rd_write_en = 0;
  logic [3:0]  rs1_addr = 0, rs2_addr = 0, rd_addr = 0;
  logic [31:0] rd_data = 0, rs1_data, rs2_data;

  reg_file dut (.*);

  always #5 clk = ~clk;

  task automatic write_reg(input logic [3:0] r, input logic [31:0] v);
    rd_addr = r; rd_data = v; rd_write_en = 1;
    @(posedge clk); #1;
    rd_write_en = 0;
  endtask
  ...
  initial begin
    ...
    for (int r = 0; r < 16; r++) begin
      ...
      #1 `CHECK_EQ(rs1_data, 32'h0, "reset clears register (port 1)")
         `CHECK_EQ(rs2_data, 32'h0, "reset clears register (port 2)")
    end
    rst_n = 1;
    ...
    for (int r = 0; r < 16; r++) begin
      ...
      #1 `CHECK_EQ(rs1_data, 32'hC0DE_0000 + r * 32'h11,        "read back on port 1")
         `CHECK_EQ(rs2_data, 32'hC0DE_0000 + (15 - r) * 32'h11, "read back on port 2")
    end
